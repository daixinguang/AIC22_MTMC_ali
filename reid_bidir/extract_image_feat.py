...
class ReidFeature():
    ...
    def __init__(self, gpu_id, _mcmt_cfg):
        ...
        self.val_transforms = T.Compose([T.Resize(self.reid_cfg.INPUT.SIZE_TEST, interpolation=InterpolationMode.BICUBIC),\
...
def save_feature(output_path, data_path, pool_output):
    ...
    all_feat_dic = {}
    for cam in os.listdir(data_path): # 根据detect的image名字去创建Key值，但是和保存的feat的key值有对不上的！！！
        dets_pkl_file = os.path.join(data_path, cam, f'{cam}_dets.pkl')
        det_dic = pickle.load(open(dets_pkl_file, 'rb')) # 先怀疑可能是检测结果和对应的切片没有对上！！！从aic21复制过来试试！！！
        all_feat_dic[cam] = det_dic.copy() # {c003: c003_dets.pkl, c004:c004_dets.pkl ...}

    for sample_dic in pool_output: # feat是并行计算的，所以len(pool_output)=数据长度/BATCH_SIZE
