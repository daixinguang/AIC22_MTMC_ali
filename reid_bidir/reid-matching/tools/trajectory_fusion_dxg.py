...
def parse_pt(pt_file,zones=None): # dxg 改写版
    ...
    for line in lines:
        ...
        if tid not in mot_list:
            mot_list[tid] = dict()
        out_dict = lines[line]
        if zones is not None: # dxg
            out_dict['zone'], out_dict['sub_zone']  = zones.get_zone(bbox)
        else:
            out_dict['zone'] = None
            out_dict['sub_zone'] = None
        mot_list[tid][fid] = out_dict
    return mot_list

def parse_pt_ori(pt_file,zones):
    if not os.path.isfile(pt_file):
        return dict()
    with open(pt_file,'rb') as f:
        lines = pickle.load(f)
...
if __name__ == '__main__':
    ...
    for cam in os.listdir(cfg.DET_SOURCE_DIR):
        cam_.append(int(cam[1:]))
    # save_dir = './exp/viz/test/S06/trajectory/'
    save_dir = f'./exp/viz/{mode}/{scenes}/trajectory/'
    # cid_bias = parse_bias(cfg.CID_BIAS_DIR, scene_name)
    cid_bias = parse_bias(cfg.CID_BIAS_DIR, [scenes])
